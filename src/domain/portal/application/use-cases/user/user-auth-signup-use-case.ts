...
export class UserAuthSignUpUseCase {
  ...
  async execute({name, login, password} : UserSignUpDTO) : Promise<UserAuthSignUpUseCaseResponse>{
    if (await this.userRepository.fetchUserByLogin(login)) return left(new LoginInUseError())
    const passwordHash = await this.encrypterGateway.encryptPassword(password)
    const newUser = await this.userRepository.createNewUser(name, login, passwordHash)
    return right({token: this.encrypterGateway.encryptToken({id: newUser.id, name: newUser.name}), user: newUser})
  }
}